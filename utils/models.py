...
if __name__ == "__main__":
    # net = clip_resnet50(trunc=2)
    # net = resnet50(pretrained="imagenet", trunc=3)
    # print(net)
    # model_names = timm.list_models(pretrained=True)
    # model_names = timm.list_models('*bit*')
    # print(model_names)
    # model = timm.create_model('resnetv2_50x1_bitm', pretrained=True)
    # print(model)

    # net, _ = bit_resnet50(trunc=4)
    # print(net)

    # net = freeze_densenet201(finetune_from=1)
    # print(net)

    # net = cbr_larget(pretrained="imagenet")
    # print(net)
    # test_input = test_input = torch.zeros(size=(2, 3, 224, 224), dtype=torch.float)
    # print(net(test_input))

    # net = densenet201(trunc=3, classes=7)
    # net = slim_densenet201(shrink_coefficient=2, load_up_to=3, num_classes=7)
    # net = freeze_densenet201(finetune_from=1, classes=7);
    
    # net = resnet50(trunc=3, classes=7)
    # print(count_parameters(net))
    # asdf
    # net = slim_resnet50(shrink_coefficient=2, load_up_to=3, num_classes=7)
    # print(count_parameters(net))
    # net = freeze_resnet50(finetune_from=1, classes=7);

    # print(count_parameters(net))
    # print(models.AlexNet())
    # asdf
    # net = alexnet(pretrained='imagenet', trunc=5, classes=2)
    # print(net)

    # net = efficientnet(num_classes=2, pretrained='imagenet', trunc=-1)
    # print(count_parameters(net))
    # net = efficientnet(num_classes=2, pretrained='imagenet', trunc=11)
    # print(count_parameters(net))

    # net = densenet201(trunc=2, classes=2)
    # print(count_parameters(net))
    # net = densenet201(trunc=-1, classes=2)
    # print(count_parameters(net))


    ## test layer wise truncation
    net = resnet50(pretrained="imagenet", trunc=13, layer_wise=False, classes=7)
    print(count_parameters(net, require_grad=False))
    print(net)
    # net = resnet50(pretrained="imagenet", trunc=-1, layer_wise=False, classes=7)
    # print(count_parameters(net, require_grad=False))

    # net = slim_resnet50(shrink_coefficient=2, load_up_to=3, num_classes=7)
    # print(count_parameters(net, require_grad=False))

    # net = layer_wise_freeze_resnet50(finetune_from=2, classes=7)
    # net = models.resnet50()
    # print(count_parameters(net, require_grad=False))
    # for name, param in net.named_parameters():
